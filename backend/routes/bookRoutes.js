...
router.post('/', async (req, res) => {
  try {
    if (!req.body.title || !req.body.author || !req.body.publishedYear || !req.body.description || !req.body.genre) {
      return res.status(400).send({ message: "Send all the required fields : title, author, publishedYear, description" });
    } else {
      const newBook = {
